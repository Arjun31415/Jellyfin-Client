...
pub async fn get_log_entries(
...
) -> Result<crate::models::ActivityLogEntryQueryResult, Error<GetLogEntriesError>> {
    ...
    if let Some(ref local_var_str) = min_date {
        local_var_req_builder =
            local_var_req_builder.query(&[("minDate", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = has_user_id {
        local_var_req_builder =
            local_var_req_builder.query(&[("hasUserId", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetLogEntriesError> =
