...
pub async fn get_configuration_pages(
...
) -> Result<Vec<crate::models::ConfigurationPageInfo>, Error<GetConfigurationPagesError>> {
    ...
    if let Some(ref local_var_str) = enable_in_main_menu {
        local_var_req_builder =
            local_var_req_builder.query(&[("enableInMainMenu", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetConfigurationPagesError> =
...
pub async fn get_dashboard_configuration_page(
    configuration: &configuration::Configuration,
    name: Option<&str>,
) -> Result<std::path::PathBuf, Error<GetDashboardConfigurationPageError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetDashboardConfigurationPageError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
