...
/// struct for typed errors of method [`get_music_genres`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetMusicGenresError {
    Status401(),
    Status403(),
    UnknownValue(serde_json::Value),
}

pub async fn get_music_genre(
    configuration: &configuration::Configuration,
    genre_name: &str,
    user_id: Option<&str>,
) -> Result<crate::models::BaseItemDto, Error<GetMusicGenreError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetMusicGenreError> =
...
pub async fn get_music_genres(
...
) -> Result<crate::models::BaseItemDtoQueryResult, Error<GetMusicGenresError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/MusicGenres", local_var_configuration.base_path);
    let mut local_var_req_builder =
        local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetMusicGenresError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
