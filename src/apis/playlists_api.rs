...
pub async fn add_to_playlist(
    ...
    user_id: Option<&str>,
) -> Result<(), Error<AddToPlaylistError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<AddToPlaylistError> =
...
pub async fn create_playlist(
    ...
    create_playlist_dto: Option<crate::models::CreatePlaylistDto>,
) -> Result<crate::models::PlaylistCreationResult, Error<CreatePlaylistError>> {
    let local_var_configuration = configuration;
...
) -> Result<crate::models::BaseItemDtoQueryResult, Error<GetPlaylistItemsError>> {
    let local_var_configuration = configuration;
...
pub async fn remove_from_playlist(
...
) -> Result<(), Error<RemoveFromPlaylistError>> {
    let local_var_configuration = configuration;
    ...
    if let Some(ref local_var_str) = entry_ids {
        local_var_req_builder = match "multi" {
            "multi" => local_var_req_builder.query(
                &local_var_str
                    .into_iter()
                    .map(|p| ("entryIds".to_owned(), p.to_string()))
                    .collect::<Vec<(std::string::String, std::string::String)>>(),
            ),
            _ => local_var_req_builder.query(&[(
                "entryIds",
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<RemoveFromPlaylistError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
